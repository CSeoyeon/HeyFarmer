...
public class recipeboardService 
{
	...
	public int write(Message msg)
	{
		try
		{
			...
			String sql = "insert into recipe values(NULL,'" + msg.getMsg_title() + "','" + 
	...
	public Message getMessage(int msg_id)
	{
		...
		try
		{
			Connection con = getConnection();
			Statement stmt = con.createStatement();
			String sql = "select * from recipe where msg_id=" + msg_id + ";";
			ResultSet rs = stmt.executeQuery(sql);
			rs.next();
	...
	public void deleteMessage(String msg_id)
	{
		try
		{
			Connection con = getConnection();
			Statement stmt = con.createStatement();
			String sql = "delete from recipe where msg_id=" + msg_id + ";";
			stmt.execute(sql);
		}
		catch(Exception e)
		{
	...
	public void updateMessage(String msg_title, String msg_content, String msg_id)
	{
		try
		{
			Connection con = getConnection();
			Statement stmt = con.createStatement();
			String sql = "update recipe set msg_title='" + msg_title + "'," +
	...
	{
		System.out.println(new recipeboardService().getDate());
	}
}
