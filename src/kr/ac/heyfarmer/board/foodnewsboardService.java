...
public class foodnewsboardService 
{
	...
	public int write(Message msg)
	{
		try
		{
			...
			String sql = "insert into foodnews values(NULL,'" + msg.getMsg_title() + "','" + 
	...
	public ArrayList<Message> getMessages()
	{
		...
		try
		{
			Connection con = getConnection();
			Statement stmt = con.createStatement();
			String sql = "select * from foodnews;";
			ResultSet rs = stmt.executeQuery(sql);
			while(rs.next())
			{
	...
	public Message getMessage(int msg_id)
	{
		...
		try
		{
			Connection con = getConnection();
			Statement stmt = con.createStatement();
			String sql = "select * from foodnews where msg_id=" + msg_id + ";";
			ResultSet rs = stmt.executeQuery(sql);
			rs.next();
	...
	public void deleteMessage(String msg_id)
	{
		try
		{
			Connection con = getConnection();
			Statement stmt = con.createStatement();
			String sql = "delete from foodnews where msg_id=" + msg_id + ";";
			stmt.execute(sql);
		}
		catch(Exception e)
		{
	...
	public void updateMessage(String msg_title, String msg_content, String msg_id)
	{
		try
		{
			Connection con = getConnection();
			Statement stmt = con.createStatement();
			String sql = "update foodnews set msg_title='" + msg_title + "'," +
	...
	{
		System.out.println(new foodnewsboardService().getDate());
	}
}
